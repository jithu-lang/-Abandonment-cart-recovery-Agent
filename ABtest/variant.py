...
def ab_test(customer_id: str):
    ...
    # Get the row belonging to this customer
    ...
    if (cart_value < 800 or 
        price_sensitivity > 0.55 or
        intent_score < 0.55 or
        abandonment_reason in price_related_reasons or
        previous_purchases < 2):
        return "A"

 
    if (cart_value > 1500 or 
        price_sensitivity < 0.40 or
        intent_score > 0.65 or
        abandonment_reason not in price_related_reasons or
        previous_purchases >= 3):
        return "B"

    # Neutral fallback (rare)
    premium_score = 0
    value_score = 0
